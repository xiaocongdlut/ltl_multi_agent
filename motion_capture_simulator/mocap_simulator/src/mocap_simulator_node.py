...
class MocapSimulatorNode:
    ...
    def load_param(self):
        ...
        self.publish_tf = rospy.get_param('~publish_tf', False)   # 获取publish_tf的值，若不存在，则默认为 False
    ...
    def setup_sub_pub(self):
        ...
        # 创建一个字典，存储所有的发布对象，根据key，找到对应的发布对象
        self.model_pub = {}
        ...
        for model in self.model_list:
            ...
            # 设置相应的消息头（header）中的坐标系（frame_id）信息
    ...
    def gazebo_model_callback(self, model_state_msg):
        ...
        # 如果当前时间 curr_time 大于上次处理回调的时间 self.prev_time 加上 self.frame_delay，则进入回调处理
...
# 将Twist消息从一个坐标系转换到另一个坐标系，并返回转换后的Twist消息
#=====================================
